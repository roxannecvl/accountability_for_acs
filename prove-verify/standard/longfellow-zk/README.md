...
Examples (from `clang-build-release` after `cmake --build`):

```
$ ./circuits/tests/ec/prove_verify_test --benchmark_filter='BM_CredentialCommitmentProveVerifyCombined_P256' --benchmark_min_time=0.1s
$ ./circuits/tests/ec/attr_commitment_experiment_test --benchmark_filter='BM_AttrSigProver_(Flat|Merkle)/16/4' --benchmark_min_time=0.1s
$ ./circuits/sha/flatsha256_circuit_test --benchmark_filter=BM_ShaZK_fp2_128
```

See `../README.md` for the npm presentation benches (prove-verify, merkle-vs-flat, communication-costs).
