...
/**
 * Longfellow (Google Benchmark): full credential commitment — flat SHA-256 over 32 attrs,
 * **no CFT** variant: 5 disclosed (indices 4,5,6,14,15); no ElGamal / c1..c4 relations.
 *
 * Targets
 * `prove_verify_no_cft_test` and writes summaries under
 * `prove-verify-no-cft/artifacts_bench_prove_verify_no_cft/`.
 */
...
} = require("../scripts/bench_gbench_common.js");
...
Environment (thesis-style):
  ...
  BENCH_FILTER          Regex, default BM_CredentialCommitmentProveVerifyNoCft.*
  BENCH_METRIC          real_time | cpu_time | both (verbose printStats)
  ARTIFACTS_DIR         Subdir under prove-verify/standard or absolute path for summaries
  LONGFELLOW_NO_CFT_BENCH_BIN   Path to prove_verify_no_cft_test (preferred in Docker)
  LONGFELLOW_CRED_BENCH_BIN     Fallback path to benchmark binary
...
function parseArgs(argv) {
  ...
  const out = {
    bin:
      process.env.LONGFELLOW_NO_CFT_BENCH_BIN ||
      process.env.LONGFELLOW_CRED_BENCH_BIN ||
      null,
    filter: process.env.BENCH_FILTER || "BM_CredentialCommitmentProveVerifyNoCftCombined_P256",
    ...
    outDir: resolveArtifactsDir("artifacts_bench_prove_verify_no_cft"),
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    if (a === "--bin") (out.bin = v), i++;
      ...
      printUsage(argv[1] || "bench_prove_verify_no_cft.js");
      process.exit(0);
    } else {
  ...
  if (!out.bin) {
    const d = defaultBinPath();
    throw new Error(
      `Benchmark binary not found at ${d}. Build first (from longfellow-zk: cmake -S lib -B clang-build-release && cmake --build clang-build-release -j), or set LONGFELLOW_NO_CFT_BENCH_BIN / LONGFELLOW_CRED_BENCH_BIN / pass --bin PATH`
    );
  }
...
function main() {
  ...
  if (!args.compact) {
    console.log(`Backend: Longfellow — prove_verify_no_cft_test (32 attrs, flat SHA, 5 used; no CFT)`);
    ...
    console.log(`Artifacts: ${args.outDir}  (env ARTIFACTS_DIR; default under prove-verify-no-cft/)`);
    console.log(
  ...
  const gbenchJsonPath = path.join(os.tmpdir(), `longfellow-cred-gbench-${process.pid}-${Date.now()}.json`);
  const measuredArgs = [
    ...benchBase,
    "--benchmark_format=console",
  ...
  const summary = buildTimingSummaryFromCombined(rows, {
    type: "longfellow",
  ...
  if (!summary) {
    console.error("No Combined prove_ns/verify_ns counters found (check --filter).");
    process.exit(1);
  }
  ...
  console.log(`Summary written: ${summaryPath}` + (args.keepArtifacts ? " (KEEP_ARTIFACTS)" : ""));
  process.exit(0);
}
