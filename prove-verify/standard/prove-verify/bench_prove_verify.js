...
/**
 * Longfellow (Google Benchmark): prove_verify (age-check + CFT) — flat SHA-256, 32 attrs.
 *
 * Environment:
 *   BENCH_N                 outer repetitions → --benchmark_repetitions (default 10). Alias: BENCH_REPETITIONS.
 *   BENCH_ITERATIONS        inner iterations per repetition → --benchmark_iterations (default 1). auto or 0 = adaptive (min_time).
 *   BENCH_MIN_TIME          passed through (default 0.05s)
 *   BENCH_FILTER            regex filter (default BM_CredentialCommitmentProveVerifyCombined_P256)
 *   BENCH_METRIC            real_time | cpu_time | both — controls --verbose printStats only (default both)
 *   ARTIFACTS_DIR           directory name or absolute path for summary JSON (default: prove-verify/artifacts_bench_prove_verify)
 *   LONGFELLOW_CRED_BENCH_BIN   path to prove_verify_test if not using default build path
 *   KEEP_ARTIFACTS          set to 1 with --keep-artifacts
 *   CLEAN                   set to 1 with --clean: delete ARTIFACTS_DIR before run
 *   BENCH_WARMUP            if 0/false/no, skip one discarded warm-up run (default: on).
 *
...
/** Stack root (`prove-verify/standard` locally, `/bench` in the Docker image). */
...
Environment (thesis-style):
  ...
  BENCH_FILTER          Regex, default BM_CredentialCommitmentProveVerify(Prover|Verifier|FullCycle)_P256
...
function parseArgs(argv) {
  ...
  const out = {
    ...
    filter:
      ...
      "BM_CredentialCommitmentProveVerifyCombined_P256",
  ...
  if (!out.bin) {
    const d = defaultBinPath();
    throw new Error(
      `Benchmark binary not found at ${d}. Build first (from longfellow-zk: cmake -S lib -B clang-build-release && cmake --build clang-build-release -j), or set LONGFELLOW_CRED_BENCH_BIN / pass --bin PATH`
    );
  }
...
function main() {
  ...
  if (!args.compact) {
    console.log(`Backend: Longfellow — prove_verify_test (32 attrs, flat SHA, 5 used; C4 = ECDSA)`);
    ...
    console.log(`Artifacts: ${args.outDir}  (env ARTIFACTS_DIR; default under prove-verify/)`);
    console.log(
