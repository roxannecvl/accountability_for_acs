...
function packedMerkleDepth(population, bitsPerLeaf) {
  const numLeaves = Math.ceil(population / bitsPerLeaf);
  const padded = nextPowerOfTwo(numLeaves);
...
function genProveVerifyRevocation({ depth, bitsPerLeaf, revocSlot }) {
...
template ProveVerifyRevocation() {
    var DEPTH = ${depth};
    var BITS = ${bitsPerLeaf};
    var REVOC_SLOT = ${revocSlot};
...
function writeGeneratedCircuits({ outDir, population, bitsPerLeaf, revocSlot, suffix = "" }) {
  const merkleDepth = packedMerkleDepth(population, bitsPerLeaf);
  const meta = { merkleDepth, population, bitsPerLeaf, revocSlot };
  const tag = suffix || "";
  const proveVerifyFile = `prove_verify_revocation${tag}.circom`;

  meta.circuitsChanged = writeIfChanged(
    path.join(outDir, proveVerifyFile),
    genProveVerifyRevocation({ depth: merkleDepth, bitsPerLeaf, revocSlot })
  );
  meta.proveVerifyFile = proveVerifyFile;
  return meta;
}

module.exports = {
  writeGeneratedCircuits,
};
