...
function ensureCppWitnessGenerator(zk, { circomFile, outDir = ".", circomBin } = {}) {
  ...
  if (binOk && datOk) {
    try {
      ...
      if (binM >= srcM && datM >= srcM) return { cppDir: cppDirRel, witnessBin: binRel };
    } catch {
      // fall through
    }
  }
...
function ensureGroth16ZkeyAndVkey(
  ...
  if (zkeyOk && vkeyOk) {
    try {
      ...
      if (zkeyM >= r1csM && vkeyM >= r1csM) return;
    } catch {
      // fall through
    }
  }
...
function prepareGroth16(
  ...
  return ensureCppWitnessGenerator(zk, {
    circomFile,
    outDir,
    circomBin,
  });
...
module.exports = {
  createZkUtils,
  ensureRapidsnarkAvailable,
  ensureCircuitCompiled,
  ensureCppWitnessGenerator,
  ensureGroth16ZkeyAndVkey,
  prepareGroth16,
};
