 ...
 * commitment circuit, **no CFT** variant: flat SHA-256 over 32 attribute slots,
 * 5 disclosed (indices 4,5,6,14,15); no ElGamal relations and no c1..c4
 * outputs. CFT counterpart: `bench_prove_verify.js`.
 *
 * Targets `prove_verify_no_cft_test` and writes summaries under
 * `prove_verify/standard/artifacts_bench_prove_verify_no_cft/` by default.
 */
...
Environment (thesis-style):
  ...
  BENCH_FILTER          Regex, default BM_ProveVerifyNoCft.*
...
function statsForSummary(arrMs) {
  ...
  const s = stats(arrMs);
  if (!s) return null;
  return { minMs: s.min, maxMs: s.max, avgMs: s.avg, p95Ms: s.p95 };
}
...
function classifyBench(base) {
  if (/^BM_ProveVerifyNoCftFullCycle/.test(base)) return "fullCycle";
  if (/^BM_ProveVerifyNoCftVerifier/.test(base)) return "verifyOnly";
  if (/^BM_ProveVerifyNoCftProver/.test(base)) return "prover";
  return "other";
}
...
function main() {
  ...
  if (!args.compact) {
    console.log(`Backend: Longfellow — prove_verify_no_cft_test (32 attrs, flat SHA, 5 used; no-CFT variant)`);
  ...
  const summary = {
    meta: {
      ...
      backend: "prove_verify_no_cft_test",
      bin: args.bin,
  ...
  for (const kind of ["prover", "verifyOnly", "fullCycle"]) {
    const pick = names.find((b) => classifyBench(b) === kind);
    if (!pick) continue;
