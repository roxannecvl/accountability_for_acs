...
class ProveVerifyCircuit {
 ...
 public:
  ...
  struct CommitWitness {
    v8 sha_in[64 * kProveVerifyCommitShaBlocks];
    ShaBlockWitness sha_bw[kProveVerifyCommitShaBlocks];
    void input(const LogicCircuit& lc) {
      for (size_t i = 0; i < 64 * kProveVerifyCommitShaBlocks; ++i) {
        sha_in[i] = lc.template vinput<8>();
      }
      for (size_t j = 0; j < kProveVerifyCommitShaBlocks; ++j) {
        sha_bw[j].input(lc);
      }
    }
  };
  ...
  ProveVerifyCircuit(const LogicCircuit& lc, const EC& ec,
                                    const Nat& order)
  ...
  void prove_relation(EltW issuer_x, EltW issuer_y, EltW elgamal_x, EltW elgamal_y,
                      ...
                      EltW c2y_pub, EltW c3x_pub, EltW c3y_pub, EltW c4_pub,
                      EltW m_pub, EltW now_pub, EltW max_birth_pub,
    ...
    Flatsha sha_commit(lc_);
    const auto nb_commit = lc_.template vbit<8>(kProveVerifyCommitShaBlocks);
    sha_commit.assert_message(kProveVerifyCommitShaBlocks, nb_commit, w.commit.sha_in,
                              w.commit.sha_bw);
    ...
    auto repack_be32 = [&](size_t ind) -> EltW {
      ...
      for (size_t i = 0; i < 32; ++i) {
        for (size_t j = 0; j < 8; ++j) {
          ...
          auto bit = lc_.eval(w.commit.sha_in[ind + i][7 - j]);
          h = lc_.add(bit, t);
        }
      }
      return h;
    ...
    EltW id_x2 = repack_be32(0);
    EltW id_y2 = repack_be32(32);
    EltW t2 = repack_be32(64);
    EltW r2Qx_aff = repack_be32(96);
    EltW r2Qy_aff = repack_be32(128);
    lc_.assert_eq(id_x2, w.id_x);
    lc_.assert_eq(id_y2, w.id_y);
    lc_.assert_eq(t2, w.t);
    lc_.assert_eq(w.t, t_pub);
    assert_equal_projective(r2QX, r2QY, r2QZ, r2Qx_aff, r2Qy_aff, one);

    lc_.vassert_eq(w.commit.sha_in[kProveVerifyCommitMsgBytes], lc_.template vbit<8>(0x80));
    for (size_t i = kProveVerifyCommitMsgBytes + 1; i < 184; ++i) {
      lc_.vassert_eq(w.commit.sha_in[i], lc_.template vbit<8>(0x00));
    }
    for (size_t i = 184; i < 190; ++i) {
      lc_.vassert_eq(w.commit.sha_in[i], lc_.template vbit<8>(0x00));
    }
    lc_.vassert_eq(w.commit.sha_in[190], lc_.template vbit<8>(0x05));
    lc_.vassert_eq(w.commit.sha_in[191], lc_.template vbit<8>(0x00));

    EltW c4_from_sha =
        repack32(sha_commit, w.commit.sha_bw[kProveVerifyCommitShaBlocks - 1].h1);
    lc_.assert_eq(c4_from_sha, c4_pub);
...
struct BenchmarkContext {
  ...
  std::array<uint8_t, 64 * kProveVerifyCommitShaBlocks> commit_sha_in{};
  std::array<FlatSHA256Witness::BlockWitness, kProveVerifyCommitShaBlocks> commit_sha_bw{};
  ...
  void build_inputs() {
    ...
    t_pub = field.to_montgomery(Nat(7777777));

    const Nat id_scalar = Nat(555555);
    auto ID = Traits::ec().scalar_multf(Traits::ec().generator(), id_scalar);
    Traits::ec().normalize(ID);
    id_x = ID.x;
    id_y = ID.y;
    ...
    // Same leaf semantics as the 16-attr Merkle credential circuit: per-slot
    // SHA256(be32(slot_index) || value_be32), then flat SHA-256 over the 32 leaves.
    ...
    const Nat r1 = Nat(123456789);
    const Nat r2 = Nat(444444444);

    // Ciphertext points.
    auto G = Traits::ec().generator();
    auto c1 = Traits::ec().scalar_multf(G, r1);
    auto c2 = Traits::ec().scalar_multf(G, r2);
    auto r1Q = Traits::ec().scalar_multf(Q, r1);
    auto r2Q = Traits::ec().scalar_multf(Q, r2);
    ...
    // Build commitment preimage: SHA256(IDx||IDy||t||r2Qx||r2Qy).
    std::array<uint8_t, kProveVerifyCommitMsgBytes> commit_msg{};
    {
      uint8_t buf[32];
      to_bytes_be(field, id_x, buf);
      std::memcpy(&commit_msg[0 * 32], buf, 32);
      to_bytes_be(field, id_y, buf);
      std::memcpy(&commit_msg[1 * 32], buf, 32);
      to_bytes_be(field, t_pub, buf);
      std::memcpy(&commit_msg[2 * 32], buf, 32);
      to_bytes_be(field, r2Q.x, buf);
      std::memcpy(&commit_msg[3 * 32], buf, 32);
      to_bytes_be(field, r2Q.y, buf);
      std::memcpy(&commit_msg[4 * 32], buf, 32);
    }

    uint8_t commit_nb = 0;
    FlatSHA256Witness::transform_and_witness_message(
        commit_msg.size(), commit_msg.data(), kProveVerifyCommitShaBlocks, commit_nb,
        commit_sha_in.data(), commit_sha_bw.data());
    c4 = sha_state_to_field(field, commit_sha_bw[kProveVerifyCommitShaBlocks - 1].h1);
...
std::unique_ptr<Circuit<typename Traits::Field>> make_circuit(size_t numInstances) {
  ...
  using EltW = typename LogicCircuit::EltW;
  using v256 = typename LogicCircuit::v256;
  using CircuitT = ProveVerifyCircuit<LogicCircuit, Field, EC>;
  ...
  std::vector<EltW> c4(numInstances);
  ...
  for (size_t i = 0; i < numInstances; ++i) {
    circuit.prove_relation(issuer_x[i], issuer_y[i], elgamal_x[i], elgamal_y[i],
                           ...
                           c3y[i], c4[i], m[i], now[i], max_birth[i], ws[i]);
  }
...
void fill_input(Dense<typename Traits::Field>& W, const BenchmarkContext<Traits>& ctx,
  ...
  for (size_t i = 0; i < 64 * kProveVerifyCommitShaBlocks; ++i) {
    filler.push_back(ctx.commit_sha_in[i], 8, Traits::field());
  }
  for (size_t b = 0; b < kProveVerifyCommitShaBlocks; ++b) {
    for (size_t k = 0; k < 48; ++k) filler.push_back(enc.mkpacked_v32(ctx.commit_sha_bw[b].outw[k]));
    for (size_t k = 0; k < 64; ++k) {
      filler.push_back(enc.mkpacked_v32(ctx.commit_sha_bw[b].oute[k]));
      filler.push_back(enc.mkpacked_v32(ctx.commit_sha_bw[b].outa[k]));
    }
    for (size_t k = 0; k < 8; ++k) filler.push_back(enc.mkpacked_v32(ctx.commit_sha_bw[b].h1[k]));
  }
...
class ProveVerifyHarnessP256 {
 public:
  explicit ProveVerifyHarnessP256(size_t numInstances)
      : ctx_(numInstances),
...
void ProveVerifyHarnessP256Deleter::operator()(
    ProveVerifyHarnessP256* p) const {
  delete p;
}

ProveVerifyHarnessP256Ptr
MakeProveVerifyHarnessP256(size_t numInstances) {
  return ProveVerifyHarnessP256Ptr(
      new ProveVerifyHarnessP256(numInstances));
}

bool ProveP256(ProveVerifyHarnessP256& h) { return h.Prove(); }

std::unique_ptr<Dense<Fp256Base>> PublicInputsP256(
    const ProveVerifyHarnessP256& h) {
  auto pub = std::make_unique<Dense<Fp256Base>>(1, /*n1=*/16);
  h.fill_public_inputs(*pub);
  return pub;
}

bool VerifyP256(const ProveVerifyHarnessP256& h,
