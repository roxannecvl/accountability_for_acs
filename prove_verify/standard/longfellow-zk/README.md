...
This trimmed copy of Longfellow ships two benchmark binaries (the upstream
`credential_commitment_proof_test`, `attr_commitment_experiment_test`, and
the SHA gadget binary are not built here). After
`cmake --build clang-build-release`:

```
$ ./circuits/tests/ec/prove_verify_test         --benchmark_filter='BM_ProveVerifyFullCycle_P256'      --benchmark_min_time=0.1s
$ ./circuits/tests/ec/prove_verify_no_cft_test  --benchmark_filter='BM_ProveVerifyNoCftFullCycle_P256' --benchmark_min_time=0.1s
```

See the thesis README at `../README.md` (and the Node helpers in
`../scripts/`) for the full prove/verify benchmark setup.
